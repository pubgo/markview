...
function SliderRow({ label, value, min, max, step, defaultValue, onChange }: SliderRowProps) {
  return (
    <label className="flex items-center gap-3">
      ...
      />
      <span className="w-10 text-right text-sm tabular-nums text-gh-text">{value}</span>
      {value !== defaultValue && (
        <button
          type="button"
      ...
      >
        ...
        <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <label className="flex items-center gap-3">
            ...
            <select
              ...
              onChange={(e) =>
                handleUpdate({
                  theme: e.target.value as MermaidSettings["theme"],
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gh-border">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className={`text-sm cursor-pointer px-4 py-1.5 rounded-md transition-colors flex items-center gap-1.5 ${
                dirty
