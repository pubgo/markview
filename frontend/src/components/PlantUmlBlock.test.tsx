...
describe("PlantUmlBlock", () => {
  ...
  it("renders svg when PlantUML conversion succeeds", async () => {
    ...
    render(<PlantUmlBlock code={["@startuml", "Alice -> Bob: Hello", "@enduml"].join("\n")} />);
  ...
  it("falls back to preformatted code when PlantUML conversion fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "图表渲染失败：渲染服务异常，请稍后重试（PlantUML render failed with status 500）。已回退为代码块显示。",
        ),
      ).toBeInTheDocument();
  ...
  it("retries rendering when clicking retry button after failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "图表渲染失败：渲染服务异常，请稍后重试（PlantUML render failed with status 503）。已回退为代码块显示。",
        ),
      ).toBeInTheDocument();
  ...
  it("keeps user custom skinparam without injecting preset", async () => {
    ...
    render(
      <PlantUmlBlock
        code={["@startuml", "skinparam ArrowColor #ff0000", "Alice -> Bob: custom", "@enduml"].join(
          "\n",
        )}
      />,
    );
