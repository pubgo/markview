...
匹配到的文件会自动打开，匹配目录也会被持续监听以发现新文件。展开与自动发现会遵守项目内的 `.gitignore`（含嵌套规则），并始终跳过 `.git/`。

命令行里的普通文件参数（含 shell 展开后的路径）**默认也会跳过** `.gitignore` 命中项；若要强行打开被忽略的文件，加 `--no-ignore`。

```console
$ markview '**/*.md'                                      # 推荐：通配即 watch（ignore 生效）
$ markview **/*.md                                        # 无引号时由 shell 展开；被 ignore 的路径仍会跳过
...
> [!TIP]
> 仍建议写 `markview '**/*.md'`：由 markview 自己扫目录并持续监听。无引号写法也能避开 `node_modules`，但不会注册 watch，新文件不会自动加入。
...
- 每次只渲染当前页，适合现场讲解
- 短封面页（仅标题，或标题 + 少量短文）会自动垂直居中
- 表格、图片、Mermaid / 代码块会按演示页尺寸缩放与排版
- 支持全屏演示与快捷键导航
- 底部进度条显示当前页/总页，点击可跳转；全屏时随控件一起显隐
- 演讲者备注：页内 HTML 注释（`<!-- ... -->`，与 Marp 兼容）会从观众画面剥离，并以备注面板展示；`N` 切换显示
- 提词器：`P` 打开独立窗口（双屏时拖到自己的显示器），与主窗翻页同步；打开后主窗备注默认关闭，避免投屏露馅
- 翻页时短淡入/上移过渡（尊重系统「减少动态效果」）
- 页内多栏：单独一行的 `|||` 将当前页拆成多列（代码块内的 `|||` 忽略）；若页首是较浅标题、栏内是更深标题，页首标题通栏显示
...
- **单文档（阅读模式）**：右侧工具列 PDF 按钮，导出当前渲染结果（一页、不截断）。
- **Slides 演示模式**：同一按钮改为导出**多页幻灯片 PDF**（逐页截取当前幻灯片面，不含备注/进度条）；文件名形如 `talk-deck.pdf`。
...
| `--no-ignore`  |      |           | 打开文件参数时不过滤 `.gitignore` |
